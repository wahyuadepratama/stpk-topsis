<?php
    $pesan = $pesan2 = $pesan3 = "";
    $alternatifrangking[0] = $hasilrangking[0] = "N/A";
    $cekalt = mysql_query("SELECT id_alternatif FROM talternatif");
    $cekkr = mysql_query("SELECT id_kriteria FROM tkriteria");
    $jalt = mysql_num_rows($cekalt);
    $jkr = mysql_num_rows($cekkr);
    $matriks = $jalt * $jkr;
    $dt = mysql_query("SELECT * FROM talternatif_kriteria");
    $jdt = mysql_num_rows($dt);
    if ($jdt==$matriks && $jdt>0) {
      $alternatif = array(); //array("Galaxy", "iPhone", "BB", "Lumia");
          ...
          for ($i=0;$i<count($alternatif);$i++)
          {
            ...
            $pesan = "Data nilai alternatif kriteria masih belum lengkap, Lengkapi data terlebih dahulu";
            $pesan2 = "Jumlah data yang sudah diinput: ".$jdt;
            $pesan3 = "Jumlah nilai yang harus diinput diinput: ".$matriks;
         }
